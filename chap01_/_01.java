...
public class _01 {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 이클립스 뷰 : 이클립스 내부에서 사용되는 작은 창들 퍼스펙티브 : 프로젝트를 개발할때 유용하게 사용할수있는 뷰들을 묶어 명칭 처음디폴트는
		 * javaee 인데 (개발자용) 입문자용으로 java를 쓰면됨
		 * 
		 * 이클립스 매뉴얼 window - show view - console등 여러 뷰들있음 window - preference - general
		 * - appearence - 창색깔바꿈 General > Editors > Text Editors > Spelling > Enable
		 * spee checking 해제 하면 맞춤법검사 안함
		 * 
		 * ctrl + i 자동정렬
		 * 
		 * 
		 * bin폴더 : 소스파일을 작성후 저장하면 javac.exe로 컴파일을 직접하지 않고도 자동으로 바이트코드로 변환후 bin에 저장된다
		 * 
		 * 혼공자 자바 강의6강- 수동 컴파일. 따라했는데 실패
		 * 
		 * 모듈이란 외부에서 재사용할수있는 패키지들을 묶은것 이클립스의 프로젝트는 하나의 모듈을 개발하는것 A모듈 B모듈 C모듈 이모여 하나의
		 * 프로그램 탄생
		 * 
		 * 모듈 기술자 (module-info.java) 해당 모듈의 이름이 뭔지, 해당 모듈이 의존하는 모듈이 뭔지, 해당 모듈을 외부에서 어떻게
		 * 사용할수있는지.
		 * 
		 * 의존하는모듈 : 해당 모듈(프로젝트)을 실행하기 위해서 필요한 외부 모듈 예를 들어 chap01모듈(프로젝트) 에는 java.base모듈이
		 * 필요하다 java.lang , java.math , jave.util 등 이 베이트모듈이다 이런 java.base모듈은 디폴트로
		 * 내장돼있어서 따로 불러올 필요없지만 java.desktop모듈이나 java.sql모듈 등 다른모듈들을 쓰려면 requires로
		 * 불러와야한다. 이때 불러오는 모듈을 의존하는모듈이라 부른다 불러올 모듈이 많으면 하나하나 불러오지않고 최상위 모듈인 java.se 모듈을
		 * requires 하면 된다.
		 * 
		 * 
		 */
		...
		// int와 문자열이 더해지면 int가 문자열로 변해서 문자열로 출력된다 /2시간
		...
		char c = '각'; // 유니코드:44033 (아스키코드일수도)
		// 유니코드 숫자로 바꿀 수 있어서 char 타입도 정수타입이다.
